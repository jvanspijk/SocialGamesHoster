<script lang="ts">
	...
	function toggleAbility(id: string) {
		...
		roleForm.abilityIds = list.includes(id) ? list.filter((i) => i !== id) : [...list, id];
	}
...
<main class="container">
	<section class="section header-section">
		<div class="input-group">
			...
			<TextInput bind:value={ruleset.name} placeholder="" name="rs-name" />
		</div>
		<div class="input-group">
	...
	<section class="section">
		...
		<div class="grid">
			{#each rolesDisplay as role (role.id)}
				<ActionCard title={role.name} description={role.description} tags={role.tags}>
					{#snippet actions()}
			...
			{:else}
				<div class="empty-state">No roles created yet.</div>
			{/each}
		</div>
	</section>
</main>
...
>
	<div class="form-container">
		<div class="form-section">
			<TextInput bind:value={roleForm.name} placeholder="Role Name" name="roleName" />
			<TextInput
	...
	{#snippet footer()}
		...
		<SecondaryButton variant="secondary" onclick={() => (roleDrawerOpen = false)}
...
>
	<div class="form-section">
		...
		<TextInput
			bind:value={abilityForm.description}
			placeholder="Description"
			multiline={true}
			rows={4}
		/>
