<script lang="ts">
	...
	async function handleDelete() {
		...
		isDeleting = true;
		try {
			await DeleteGameSession(fetch, { gameId: selectedGameId });
		...
		<tr>
			...
			<td>
				<div class="actions-wrapper">
					<SecondaryButton onclick={() => goto(resolve(`/admin/games/${game.id}`))}
						>Manage</SecondaryButton
					>
					<SecondaryButton variant="danger" onclick={() => openDeleteModal(game.id)}
