...
func Register(event *core.ServeEvent) {
	...
	group.POST("/games/{id}/open-joining", openJoining).BindFunc(actorauth.RequireGameMaster)
	group.POST("/games/{id}/close-joining", closeJoining).BindFunc(actorauth.RequireGameMaster)
